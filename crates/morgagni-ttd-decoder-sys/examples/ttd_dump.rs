...
#[derive(Clone, Copy)]
struct Position { sequence: u64, steps: u64 }
...
#[derive(Serialize)]
struct PositionRange { min: Position, max: Position }
...
struct Amd64Registers {
    rax: String, rcx: String, rdx: String, rbx: String,
    rsp: String, rbp: String, rsi: String, rdi: String,
...
struct RegisterDelta {
    #[serde(skip_serializing_if = "Option::is_none")] rax: Option<String>,
    #[serde(skip_serializing_if = "Option::is_none")] rcx: Option<String>,
    #[serde(skip_serializing_if = "Option::is_none")] rdx: Option<String>,
    #[serde(skip_serializing_if = "Option::is_none")] rbx: Option<String>,
    #[serde(skip_serializing_if = "Option::is_none")] rsp: Option<String>,
    #[serde(skip_serializing_if = "Option::is_none")] rbp: Option<String>,
    #[serde(skip_serializing_if = "Option::is_none")] rsi: Option<String>,
    #[serde(skip_serializing_if = "Option::is_none")] rdi: Option<String>,
    #[serde(skip_serializing_if = "Option::is_none")] r8:  Option<String>,
    #[serde(skip_serializing_if = "Option::is_none")] r9:  Option<String>,
    #[serde(skip_serializing_if = "Option::is_none")] r10: Option<String>,
    #[serde(skip_serializing_if = "Option::is_none")] r11: Option<String>,
    #[serde(skip_serializing_if = "Option::is_none")] r12: Option<String>,
    #[serde(skip_serializing_if = "Option::is_none")] r13: Option<String>,
    #[serde(skip_serializing_if = "Option::is_none")] r14: Option<String>,
    #[serde(skip_serializing_if = "Option::is_none")] r15: Option<String>,
    #[serde(skip_serializing_if = "Option::is_none")] rip: Option<String>,
    #[serde(skip_serializing_if = "Option::is_none")] eflags:  Option<String>,
}

fn compute_delta(prev: &sys::DhttdAmd64Registers, curr: &sys::DhttdAmd64Registers) -> RegisterDelta {
...
fn convert_range(r: sys::DhttdPositionRange) -> PositionRange {
    PositionRange { min: convert_position(r.min), max: convert_position(r.max) }
}

fn convert_registers(r: sys::DhttdAmd64Registers) -> Amd64Registers {
    let h = |v: u64| format!("{v:x}");
    Amd64Registers {
        rax: h(r.rax), rcx: h(r.rcx), rdx: h(r.rdx), rbx: h(r.rbx),
        rsp: h(r.rsp), rbp: h(r.rbp), rsi: h(r.rsi), rdi: h(r.rdi),
        r8:  h(r.r8),  r9:  h(r.r9),  r10: h(r.r10), r11: h(r.r11),
        r12: h(r.r12), r13: h(r.r13), r14: h(r.r14), r15: h(r.r15),
        rip: h(r.rip),
...
fn main() -> std::io::Result<()> {
    ...
    while let Some(a) = it.next() {
        match a.to_str() {
            ...
            Some("--max-steps") => {
                ...
                max_steps = v.to_string_lossy().parse().expect("--max-steps must be a positive integer");
            }
            Some("--keyframe-interval") => {
                let v = it.next().expect("--keyframe-interval requires a value");
                keyframe_interval = v.to_string_lossy().parse().expect("--keyframe-interval must be a positive integer");
                assert!(keyframe_interval >= 1, "--keyframe-interval must be >= 1");
            }
            Some("-h") | Some("--help") => {
                eprintln!("usage: ttd-dump <trace.run> [out.json] [--whole] [--max-steps N] [--keyframe-interval N]");
                return Ok(());
            }
            _ => positional.push(a),
        }
    ...
    let trace = pit.next().expect("usage: ttd-dump <trace.run> [out.json] [--whole]");
    ...
    unsafe { sys::dhttd_engine_get_system_info(handle, &mut sysi); }
    ...
    unsafe { sys::dhttd_engine_get_lifetime(handle, &mut lifetime); }
    ...
    for i in 0..thread_count {
        ...
        if unsafe { sys::dhttd_engine_thread(handle, i, &mut t) } == 0 { continue; }
        threads.push(Thread {
    ...
    if !has_cursor {
        eprintln!("warning: NewCursor failed (0x{cur_rc:08x}); skipping per-exception register/memory snapshots");
    }

    for i in 0..exc_count {
        let mut ev = sys::DhttdException::default();
        if unsafe { sys::dhttd_engine_exception(handle, i, &mut ev) } == 0 { continue; }
        ...
        let context = if has_cursor {
            unsafe { sys::dhttd_cursor_set_position(cursor, ev.position); }
            ...
            if ev.record_address != 0 {
                let mut buf = [0u8; 8];
                let n = unsafe {
                    sys::dhttd_cursor_read_memory(cursor, ev.record_address, buf.as_mut_ptr(), buf.len())
                };
                for b in &buf[..n] {
                    use std::fmt::Write;
    ...
    if has_cursor {
        unsafe { sys::dhttd_cursor_destroy(cursor); }
    }
    ...
    let mut kf_set: std::collections::HashSet<(u64, u64)> = std::collections::HashSet::with_capacity(kf_count);
    for i in 0..kf_count {
        let mut p = sys::DhttdPosition::default();
        if unsafe { sys::dhttd_engine_keyframe(handle, i, &mut p) } == 0 { continue; }
    ...
    if whole {
        for t in &threads {
            ...
            if rc != 0 || wcur == 0 {
                eprintln!("warning: NewCursor failed for thread {} (0x{rc:08x}); skipping", t.unique_id);
                continue;
            }

            let start = sys::DhttdPosition { sequence: t.active_time.min.sequence, steps: t.active_time.min.steps };
            let end   = sys::DhttdPosition { sequence: t.active_time.max.sequence, steps: t.active_time.max.steps };
            unsafe { sys::dhttd_cursor_set_position_on_thread(wcur, t.unique_id, start); }
            ...
            loop {
                ...
                if unsafe { sys::dhttd_cursor_get_position(wcur, &mut cur) } == 0 { break; }
                ...
                if regs_ok {
                    ...
                    let payload = match (since_keyframe, last_regs.as_ref()) {
                        ...
                        (_, Some(prev_regs)) => {
                            StepPayload::Delta(compute_delta(prev_regs, &regs))
                        }
                    };
                ...
                if (cur.sequence, cur.steps) >= (end.sequence, end.steps) { break; }
                ...
                if prev == Some(now) { break; }
                prev = Some(now);
            }

            unsafe { sys::dhttd_cursor_destroy(wcur); }

            let step_count = steps.len();
            recorded_regions.push(RecordedRegion {
                anchor: RegionAnchor::ThreadWhole { thread_unique_id: t.unique_id },
